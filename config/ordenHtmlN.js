...
module.exports.HTML = function (valores, partidas, imagen, user) {
    ...
    cHtml += "<link href='styles/StyleSheetN.css' rel='stylesheet' />"
    cHtml += "<head>"
    cHtml += "<title>Orden " + valores.Serie + '-' + valores.Orden + "</title>"

    cHtml += "</head>"
    cHtml += "<body style='max-width:800px;' onLoad='setTimeout(window.print(), 800)' >"
    ...
    cHtml += "<div class='Fila'><img src='Imagenes/icon-ios7-cart-outline.svg'  class='img1'/></div><strong>Refacciones y/o Servicios --- Venta:" + valores.VENTA  + "</strong>"
    ...
    if (valores.Garantia == 0){
        ...
        cHtml += "<div style='padding-top:10px; font-size:10px'>*Este monto puede cambiar dependiendo del Diagnóstico y reparación del Equipo.</div>"
    ...
    var password = nombre[0] + nombre[1] + apellido[0] + valores.Serie + valores.id
     ...
     //------Firma
    cHtml += "<table style='padding-bottom:16px;'>"
    cHtml += "<tr><th  style='width:380px'>Acepto Terminos y Condiciones<br />  Nombre y Firma <br /> " + valores.Contacto + " </th>"
    cHtml += "<th style ='text-align:center; color:crimson;'>Puede consultar su ORDEN DE SERVICIO en la siguiente pagina: <br /> GRUPOCFAPP.COM/SERVICIO <br /> Su contraseña es: " + password +  "</th></tr>"
    cHtml += "</table> "
     //------Firma
    ...
    cHtml += "<P style='margin-top:5px;'>En Ezequiel Montes,Qro el dia: " + dateformat(valores.Fecha_ing) + ", " + valores.Contacto
    cHtml += "<p style='margin-top:0px; margin-bottom:0px;'><strong>Términos y Condiciones</strong></p>"
    ...
    if (valores.AutRepara === 1) {
        cHtml += "<div class='section'><input type='checkbox' name='Aut2' id='Checkbox2' /><label for='Checkbox2'><span ><img src='Imagenes/icon-checkmark-round.svg' /></span>Autorizo reparación</label>"
    } else {
        cHtml += "<div class='section'><input type='checkbox' name='Aut2' id='Checkbox2' checked='checked' /><label for='Chechbox2'><span ></span>Autorizo reparación</label>"
    }
    cHtml += "</div></div>"
    cHtml += "<table>"
...
function dateformat(fecha) {
    ...
    var mes = monthNames[date.getMonth() + 1];
