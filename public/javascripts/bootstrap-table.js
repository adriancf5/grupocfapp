...
!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        formatNoMatches: function () {
            return 'Ningun elemento coincide con su busqueda';
        },
        ...
        //onDblClickRow: function (item, $element) { alert($element.attr('data-index')); },
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        $.each(this.options.columns, function (i, column) {
            ...
            that.header.id.push(column.id);
            that.header.cliente.push(column.CLIENTE);
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.add ) {
           // html.push(sprintf('<button class="btn btn-default" type="button" name="add" title="%s">',
           //     this.options.formatAdd()),
        ...
        if (this.options.add) {
            this.$toolbar.find('button[name="add"]')
                .off('click').on('click', function () {
                    var target = $('.tab-pane.active').find('table').attr('data-select-item-name');
    ...
    BootstrapTable.prototype.initBody = function (fixedScroll) {
        ...
        for (var i = this.pageFrom - 1; i < this.pageTo; i++) {
            ...
            html.push('<tr',
                sprintf(' value="%s"', item.id || item.CLIENTE),
            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
                    ...
                    //text = ['<td class="bs-checkbox" style="width:80px">',
                    //    '<input' +
                    //        sprintf(' data-index="%s"', i) +
                    ...
                    //         ' /><div class="ion-flag icono"></div> <div class="ion-edit icono"></div><div class="ion-document icono"> </div>',
                    //     '</td>'].join('');
                    ...
                    if (item.Orden) {
                        //var image = '';
                        //if (!item.TipoEquipo === 'Servicio') {
                        var image = sprintf('<div href="#Imagen" role="button" data-toggle="modal" name=%s value=%s class="ion-android-image  icono"></div>', item.Serie, item.Orden)
                        style = "width:90px";
                        //} 
                        actionIcon = sprintf('<div href="#" data-get="/Seguimi" value=%s  name=%s class="ion-flag icono %s "></div>', item.id, item.Serie + '-' + item.Orden, item.estatus) + 
                                     image + 
                                    //sprintf('<div class="ion-document icono"></div>' ) +
                                    sprintf(' <div data-get="Orden" value="%s"  name="%s" class="ion-edit icono"> </div>', item.id, item.Serie + '-' + item.Orden)

                    } else if (item.CLIENTE) {
                        actionIcon = sprintf(' <div data-get="Client" value="%s"  name="%s" class="ion-edit icono"> </div>', item.CLIENTE, item.CLIENTE)
                                     //sprintf('<div class="ion-ios7-trash-outline icono"></div>') +          
                        style = "width:50px";

                    } else if (item.ARTICULO) {
                        actionIcon = sprintf(' <div data-get="Producto" value="%s"  name="%s" class="ion-edit icono"> </div>', item.ARTICULO, item.ARTICULO)
                        style = "width:50px";

                    } else if (item.Usuario && item.password) {
                        actionIcon = sprintf(' <div data-get="Usuario" value="%s"  name="%s" class="ion-compose icono"> </div>', item.Usuario, item.Usuario)
                        style = "width:50px";
                    } else if (item.Usuario) {
                        actionIcon = sprintf(' <div data-get="Usuario" value="%s"  name="%s" class="ion-edit icono"> </div>', item.Usuario, item.Usuario)
                        style = "width:50px";

                    } else if (item.Atendio){
                         
                         actionIcon = sprintf(' <div data-get="ServicioSis" value="%s"  name="%s" class="ion-edit icono"> </div>', item.id, 'Rep-' + item.id )
                        style = "width:50px";    
                    }
                        ...
                        text = ['<td class="bs-checkbox" style=' + style +'>',
                        '<div ' +
                            sprintf(' data-index="%s"', i) +
                            sprintf(' name="%s"', that.options.selectItemName) +
                            '> </div>' + actionIcon ,
