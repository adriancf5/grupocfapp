                ...
                if (that.options.columns[j].checkbox || that.options.columns[j].radio) {
                    ...
                    if (item.Orden) {
                        ...
                        actionIcon = sprintf('<div href="#" data-get="/Seguimi" value=%s  name=%s class="ion-flag icono %s "></div>', item.id, item.Serie + '-' + item.Orden, item.estatus) + 
                                    ...
                                    sprintf(' <div href="#Orden" role="button"  data-toggle="modal" value="%s"  name="%s" style="font-size:1.6em!important" class="ion-document icono"> </div>', item.id, item.Serie + '-' + item.Orden)

                    } else if (item.CLIENTE) {
                        actionIcon = sprintf(' <div data-get="Client" value="%s"  name="%s" class="ion-compose icono"> </div>', item.CLIENTE, item.CLIENTE)
